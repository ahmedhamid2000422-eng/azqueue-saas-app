...
/**
 * StaffDashboard — single-page view for a staff member at a branch.
 *
 * Three states:
 *   · idle   — no current customer; "Pull next customer" or "Take a break"
 *   · serving — customer assigned; Complete / Skip / Pass / Status
 *   · break  — calm overlay with elapsed time + Back to it
 *
 * Lean by design — staff see their work and nothing else. No insights,
 * no settings, no other staff's queues. RLS already enforces this.
 */
export default function StaffDashboard() {
  ...
  const reload = useCallback(async () => {
    ...
    const [{ data: tk }, { data: stf }, { count: doneCount }] = await Promise.all([
      supabase.from("tickets")
        .select("id, token, status, customer_name, service_id, staff_id, priority, source, created_at, called_at, started_at, completed_at, branch_id, notes")
  ...
  useEffect(() => {
    if (!primary?.branch_id) return;
    const ch = supabase
      .channel(`staff-${primary.id}-tickets`)
      .on("postgres_changes",
  ...
  async function complete() {
    ...
    logServiceTime({
      branchId: myCurrent.branch_id, ticketId: myCurrent.id, serviceId: myCurrent.service_id,
      staffId: myCurrent.staff_id, startedAt: myCurrent.started_at, completedAt: ts,
    });
  ...
  // Send current customer back to the waiting queue
  ...
  if (membershipLoading) {
    return <Shell><div className="text-center py-20 ovline text-ink-mute">Loading…</div></Shell>;
  }
  if (!primary) {
    // Not actually staff anywhere — bounce
  ...
  // Break view replaces everything
  if (myStatus === "on_break") {
    const since = primary.status_since
      ? Math.floor((now - new Date(primary.status_since)) / 60000)
      : 0;
    return (
      <Shell branch={branch} primary={primary} signOut={endShift} status={myStatus} setStatus={setStatus} disabled={busy}>
        <div className="atmosphere-hero atmosphere-prayer min-h-[calc(100vh-72px)] flex items-center justify-center px-6">
          <Card luxe variant="sage" className="p-12 max-w-md text-center">
            ...
            <div className="rule-ornament my-5 text-[8px]"><span>·</span></div>
            <Button onClick={endBreak} disabled={busy} size="lg" className="w-full">
              Back to it →
            </Button>
  ...
  return (
    <Shell branch={branch} primary={primary} signOut={endShift} status={myStatus} setStatus={setStatus} disabled={busy}>
      ...
      <div className="atmosphere-hero min-h-[calc(100vh-72px)] flex items-center justify-center px-6 py-8">
  ...
  return (
    <div className="relative corner-marks luxe-panel border border-line p-10">
      ...
      <div className="grid grid-cols-3 gap-2">
        <Button variant="ghost" onClick={onSendBack} disabled={disabled} className="w-full">
          Park
        </Button>
        <Button variant="ghost" onClick={onSkip} disabled={disabled} className="w-full">
          Skip
        </Button>
              ...
              <div className="absolute right-0 top-full mt-1 w-56 luxe-panel border border-line shadow-lg z-50">
                ...
                {staffList.map((s) => (
                  <button
                    ...
                    className="w-full text-left px-4 py-2.5 text-xs hover:bg-surface-2 transition flex items-center justify-between"
                  >
                    <div>
    ...
    <div className="space-y-3">
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 border text-[10px] tracking-[0.18em] uppercase transition ${
                active ? "bg-[rgba(201,168,106,0.06)] border-gold-deep text-gold" : `${s.color} hover:border-gold-deep`
              }`}
            >
      ...
      <div className="grid grid-cols-3 gap-px bg-line border border-line">
        ...
        <div className="bg-bg-elev p-3 text-center">
          ...
          <div className="font-display text-base mt-1 capitalize gold-text-soft">{status === "on_break" ? "Break" : status}</div>
        </div>
      </div>
    </div>
  );
...
/* ── Shell with branch header + sign out ─────────────────────────── */
function Shell({ children, branch, primary, signOut, status, setStatus, disabled }) {
  return (
    <div className="min-h-screen bg-bg text-ink flex flex-col">
      <header className="px-6 py-4 border-b border-line flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-display text-base">{branch?.name ?? "AzQueue"}</div>
            {primary?.display_name && (
              <div className="text-[10px] text-ink-mute tracking-wide">{primary.display_name} · {primary.role}</div>
            )}
          </div>
        </div>
        {primary && (
          <div className="flex items-center gap-3">
            <span className="ovline text-[9px] text-ink-mute hidden sm:inline">
              <span className={`pip mr-1.5 inline-block ${status === "active" || status === "serving" ? "breathe" : ""}`}
                    style={{ background: status === "on_break" ? "#74b9e8" : status === "off" ? "#6e6c65" : "#9bbd9b" }} />
              {status === "on_break" ? "On break" : status === "serving" ? "In service" : status === "active" ? "Ready" : "Off"}
            </span>
            <Button variant="ghost" size="sm" onClick={signOut} disabled={disabled}>End shift</Button>
          </div>
        )}
      </header>
      {children}
